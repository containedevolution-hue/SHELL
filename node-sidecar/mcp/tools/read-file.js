...
module.exports = {
  ...
  definition: {
    ...
    function: {
      name: 'read_file',
      description:
        'Read a UTF-8 text file from the appliance filesystem. ' +
        'Paths may be absolute or relative to MCP_ROOT. ' +
        'Files larger than 1 MiB are truncated; binary files return an error. ' +
        'Use list_directory first to discover paths.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'File path (absolute or relative to MCP_ROOT).' },
        },
        required: ['path'],
      },
    },
