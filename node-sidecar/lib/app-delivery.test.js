...
test('explicit update atomically switches release and keeps the stable browser origin, previous bytes and data',async t=>{
  ...
  let store=await(await fetch(origin+'/v1/app-store')).json();
  assert.equal(store.apps[0].updateAvailable,true);
  const changed=fixture('0.3.0');
  fs.writeFileSync(path.join(catalog,'notes.ceapp.json'),changed.bytes);
  fs.writeFileSync(path.join(catalog,'catalog.json'),JSON.stringify({contractVersion:1,apps:[{...changed.app,file:'notes.ceapp.json'}]}));
  const stale=await fetch(origin+'/v1/app-store/notes/install',{method:'POST',headers:{'X-Shell-Install':store.installToken,Origin:origin}});
  assert.equal(stale.status,409);assert.equal((await stale.json()).error,'release_changed');
  assert.equal(registry.list()[0].version,'0.1.0');
  fs.writeFileSync(path.join(catalog,'notes.ceapp.json'),next.bytes);
  fs.writeFileSync(path.join(catalog,'catalog.json'),JSON.stringify({contractVersion:1,apps:[{...next.app,file:'notes.ceapp.json'}]}));
  store=await(await fetch(origin+'/v1/app-store')).json();
