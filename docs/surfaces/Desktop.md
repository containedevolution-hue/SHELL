...
The current CEE OS desktop layout: one OS-owned Status Bar, a fixed centered Home control, and Core represented as a black-and-blue ember over an aerial rippled mesh.

**Built:**
- Layout implemented: a shared top Status Bar with context-sensitive menus/settings; a centered Home control (globe plus home symbol plus square outline) that stays fixed across every page and never closes running apps; Core as the CEE OS ember over the full-screen mesh surface
