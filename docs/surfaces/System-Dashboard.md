...
- Sleep timing is selectable per app beyond the two-minute default and includes Never; a master Never sleep any app control is available. Checkpoint-capable Powerhouses and containers place an obvious manual Checkpoint control in the Status Bar while active.
- The center Status Bar control is the blue Core ember labeled Core and opens Core rather than acting as a generic Home button. Core offers a one-click external-drive backup entry point.
