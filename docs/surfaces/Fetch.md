...
This document defines the accepted ownership and target behavior. A complete FETCH browser/Lab runtime and the Chat-to-Lab handoff are not shipped in Shell. The handoff requires an explicit versioned contract and runtime acceptance before either surface advertises it as available.
...
- [Apps Chat](../../../Apps/apps/chat/README.md) owns the general CE conversation surface and its conversation state. FETCH keeps ordinary browser and research controls and owns research artifacts, evidence, permissions and execution; it does not create another general chat window. Provider-owned conversations and Tenari's Companion remain within their respective owners.
- A Lab is one research assignment with its own permanent account-scoped sequential identifier such as `Lab 002`; identifiers are never reused, and a title may be renamed without changing the identifier. Its identifier is distinct from any linked Chat conversation identifier.
...
- Every raw research body is sealed to its owning store's key under the future storage contract. Index taxonomy and the minimum fields needed to locate a surviving standardized research record remain searchable without exposing raw research. This target does not claim that Shell has implemented encryption or that a Chat link transfers access to its conversation.
...
- An explicit research action creates or resumes a Lab; starting an unrelated Chat conversation does not create one. A future app-to-Chat contract will link the Lab and conversation by owner-issued identifiers, carry only selected bounded context and provenance, and provide a return destination. A reference is neither a transcript copy nor an execution grant. One Lab may move through the laboratory while retaining its identifier, Library, and research record.
...
- Deleting a Lab removes Shell-owned raw research, working data, and Lab Library. Its standardized research record, taxonomy, evidence provenance, and Index presence survive as a dead Lab record so prior research remains findable. The Index distinguishes active, archived, and deleted-source Lab identifiers. Lab deletion does not delete a linked Chat or provider conversation, and deleting a conversation does not delete the Lab. Cross-owner references must represent missing/deleted sources through the future handoff contract; they cannot silently restore content or invoke another owner's deletion operation.
...
- A Lab Library holds that assignment's living summary, findings and claims, evidence and sources, tables and comparisons, generated research artifacts, open questions, taxonomy, and category history. Conversation state remains with its conversation owner; the Lab holds bounded research context and provenance rather than a second canonical transcript.
...
- Deletion freezes the last standardized record and removes Shell-owned raw research inputs, fetched page bodies, working snippets, and private scratch state. The surviving record keeps only its structured synthesis, citations, source metadata, taxonomy, and evidence assessment. This lifecycle governs research data only, not another owner's messages.
...
- Contractor review and keep/correct rules remain owned by [Apps Contractors](../../../Apps/apps/contractors/README.md).
...
The research execution order lives in [Fetch Research Lab](../build-orders/Fetch-Research-Lab.md). [CE Apps Mobile and Chat](../../../Apps/memory/ledger/build-orders/CE-Mobile-and-Chat.md) owns the general conversation composition; a future published app-to-Chat handoff must preserve the boundaries above. A build-order description alone is not runtime proof.
