...
This is not live Windows window control. The JavaScript backend requires an explicitly accepted native driver, and none is shipped or exposed through Tauri, HTTP, or page IPC. A future Rust driver must implement the fixed operations, revalidate native identity around each call, and remain behind the authenticated native bridge. The production provider registry stays empty.
...
Windows additionally requires the trusted Rust driver, authenticated caller wiring, physical mixed-DPI/multi-monitor placement, operating-system focus behavior, package update/restart handling, recoverable minimization, exact A-B-A return, crash recovery, remote-session survival, and a side-by-side capability comparison. Installed package observations are discovery evidence, not acceptance. Until those platform gates pass, Windows also reports the native desk manager unavailable.
