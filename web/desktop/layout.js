(() => {
  ...
  function activitySurface(section){
    const center={Status:['System status','Core is protected. Preview measurements show how the surrounding gauges remain visible while the center changes.'],Warnings:['Warnings','Warnings explain the pressure, responsible workload, likely effect and safe actions.'],Workflows:['Ordinary workflows','Running work is grouped by the outcome the user started, not exposed as unexplained process gibberish.'],Permissions:['Resource permissions','Choose what a workload may read, write, reserve, borrow and use before applying the change.'],Limits:['Resource limits','Warnings and hard limits remain separate; every limit states whether it will slow, queue, pause or stop work.'],Priorities:['Priority and background tasks','Protected Core services are listed separately from optional background work, with a plain explanation of why each must run.']}[section];
    ...
    return container(`<div class="instrument-board"><div class="gauge-rail" aria-label="Resource speedometers">${gauges.slice(0,3).map(meter).join('')}</div><section class="instrument-center"><p class="eyebrow">${section}</p><h2>${center[0]}</h2><p>${center[1]}</p><span class="preview-pill">Preview data</span></section><div class="gauge-rail" aria-label="Health speedometers">${gauges.slice(3).map(meter).join('')}</div></div>`);
  }
