(() => {
  ...
  function closeFlyout(restore=false){$('flyout').hidden=true;for(const id of ['notifications','active-apps'])$(id).setAttribute('aria-expanded','false');if(restore&&flyoutTrigger)flyoutTrigger.focus();flyoutTrigger=null;}
  ...
  function openActiveApps(){showFlyout($('active-apps'),'Active apps',`<p>Visible does not mean running. Choose when each app sleeps and resumes.</p><section class="active-app"><div><strong>Scribble</strong><small>Visible · sleeping after autosave</small></div><label><input type="radio" name="scribble-policy" checked> Pause when inactive</label><label><input type="radio" name="scribble-policy"> Pause when minimized</label><label><input type="radio" name="scribble-policy"> Pause after a delay</label><label><input type="radio" name="scribble-policy"> Always on</label></section><section class="active-app"><div><strong>Automations</strong><small>Background permission required</small></div><label><input type="radio" name="automation-policy"> Pause when inactive</label><label><input type="radio" name="automation-policy" checked> While its Powerhouse is active</label><label><input type="radio" name="automation-policy"> While this computer is on</label></section><p class="preview-note">Preview only — no process policy is changed.</p>`,true);}
  ...
  $('home').onclick=()=>go('desktop');$('open-core').onclick=()=>go('core');$('active-apps').onclick=openActiveApps;$('notifications').onclick=openNotifications;$('close-flyout').onclick=()=>closeFlyout(true);
