...
$machine = if ($Accelerator -eq 'Whpx') { 'q35,accel=whpx,kernel-irqchip=off' } else { 'q35,accel=tcg' }
...
$qemuArgs = @(
    ...
    '-vga', 'std',
    '-display', 'gtk,clipboard=on,grab-on-hover=off,show-cursor=on,zoom-to-fit=on',
