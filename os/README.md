...
- The remaining privileged broker, native surfaces, independent session, startup experience, and physical MSI qualification live only in the workspace-root next-items queue
- The adopted Btrfs one-two-three recovery contract and proof: (1) an immutable clean Powerhouse source, (2) the last successfully closed state used for the next launch, and (3) a quarantined Session Trail using exactly two alternating automatic savepoints, where each event replaces the older point so a corrupted newest point retains one immediate predecessor. A manual Status Bar checkpoint is available while a checkpoint-capable workload is active. A successful close validates and promotes the final session state to the next last-good state; failure can use the prior point, abandon the session for the prior last-good state, or reset to the clean source. App documents remain outside this rotation.
- The adopted app-data attachment: first launch requests one scoped writable stub/drop-box folder from Core, creates a machine-readable manifest and plain README explaining purpose, permissions, formats, backup, removal, and restore, and receives no access to sibling Core data. Data the user places in that stub is an explicit grant; the app may transform it and maintain only its own required logs and memory there. Removing or reverting the app leaves this data intact.
