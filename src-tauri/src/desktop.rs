...
const CORE_FOLDER_NAME: &str = "Contained Evolution Core";
const CORE_MANIFEST_VERSION: u32 = 1;
const CORE_COMPARTMENTS: [&str; 7] = [
    "Apps",
    "Personal",
    "Seed",
    "Devices",
...
pub struct StorageStatus {
    platform_supported: bool,
    this_device: Option<StorageDevice>,
    removable: Vec<RemovableVolume>,
    problem: Option<String>,
}
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
...
fn new_core_id() -> String {
    let mut bytes = [0u8; 16];
    let _ = getrandom::getrandom(&mut bytes);
    bytes.iter().map(|b| format!("{b:02x}")).collect()
}
fn manifest_path(root: &Path) -> PathBuf {
    root.join("core.manifest.json")
}
fn read_manifest(root: &Path) -> Result<CoreManifest, String> {
    let text = fs::read_to_string(manifest_path(root))
        .map_err(|_| "No Core manifest was found at this location.".to_string())?;
    let manifest: CoreManifest = serde_json::from_str(&text)
        .map_err(|_| "The Core manifest could not be read.".to_string())?;
    if manifest.schema_version != CORE_MANIFEST_VERSION {
        return Err("This Core was created by an incompatible version.".into());
    }
    Ok(manifest)
}
fn create_core(parent: &Path) -> Result<(PathBuf, CoreManifest), String> {
    if !parent.is_dir() {
        return Err("Choose a folder.".into());
    }
    let root = parent.join(CORE_FOLDER_NAME);
    if let Ok(existing) = read_manifest(&root) {
        return Ok((root, existing));
    }
    if root.is_dir() && fs::read_dir(&root).map_err(problem)?.next().is_some() {
        return Err(
            "A Contained Evolution Core folder already exists here without a valid manifest."
                .into(),
        );
    }
    for compartment in CORE_COMPARTMENTS {
        fs::create_dir_all(root.join(compartment)).map_err(problem)?;
    }
...
fn core_status_at(root: Option<PathBuf>) -> CoreStatus {
    match root {
        None => CoreStatus {
            connected: false,
            path: None,
        ...
        Some(root) => match read_manifest(&root) {
            Ok(manifest) => CoreStatus {
                connected: true,
                path: Some(display(&root)),
                core_id: Some(manifest.core_id),
                problem: None,
            },
            Err(problem) => CoreStatus {
                connected: false,
                path: Some(display(&root)),
                core_id: None,
...
fn core_pointer_path(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    app.path()
        .app_local_data_dir()
        .map(|dir| dir.join("core-root.json"))
        .map_err(|_| "The local application data location is unavailable.".to_string())
}
fn read_core_pointer(app: &tauri::AppHandle) -> Option<PathBuf> {
    let pointer = core_pointer_path(app).ok()?;
    let text = fs::read_to_string(pointer).ok()?;
    let value: serde_json::Value = serde_json::from_str(&text).ok()?;
    Some(PathBuf::from(value.get("path")?.as_str()?))
}
fn write_core_pointer(app: &tauri::AppHandle, root: &Path) -> Result<(), String> {
    let pointer = core_pointer_path(app)?;
    if let Some(parent) = pointer.parent() {
        fs::create_dir_all(parent).map_err(problem)?;
    }
    fs::write(&pointer, serde_json::json!({ "path": display(root) }).to_string()).map_err(problem)
}
...
#[cfg(any(target_os = "linux", test))]
fn parse_device_usage(rows: &str) -> Option<StorageDevice> {
    let data_line = rows.lines().nth(1)?;
...
#[cfg(any(target_os = "linux", test))]
fn parse_removable_volumes(rows: &str) -> Vec<RemovableVolume> {
    rows.lines()
        .filter_map(|line| {
            let mut name = None;
            ...
            for field in line.split_whitespace() {
                ...
                match key {
                    ...
                    "SIZE" => size_bytes = value.parse::<u64>().ok(),
                    "RM" => removable = value == "1",
                    "MOUNTPOINT" if !value.is_empty() => mount_point = Some(value.to_string()),
                    _ => {}
                }
            }
            if !removable {
...
#[cfg(target_os = "linux")]
fn linux_storage_status() -> StorageStatus {
    let device_output = output("df", &["-B1", "--output=size,used,avail", "/"]);
    let this_device = device_output.as_deref().and_then(parse_device_usage);
    let problem = this_device
        .is_none()
        .then(|| "Disk usage could not be read.".into());
    let removable_output = output("lsblk", &["-b", "-o", "NAME,SIZE,RM,MOUNTPOINT", "-P"]);
    let removable = removable_output
        .as_deref()
        .map(parse_removable_volumes)
        .unwrap_or_default();
    StorageStatus {
        platform_supported: true,
        this_device,
...
fn linux_storage_status() -> StorageStatus {
    StorageStatus {
        ...
        this_device: None,
        removable: Vec::new(),
        problem: Some("Native storage evidence is connected on CEE OS Linux.".into()),
    }
}
...
#[tauri::command]
pub async fn desktop_storage_status(window: WebviewWindow) -> Result<StorageStatus, String> {
    authorize(&window)?;
    tauri::async_runtime::spawn_blocking(linux_storage_status)
        .await
        .map_err(|_| "Storage evidence collection was interrupted.".to_string())
}
#[tauri::command]
pub fn desktop_core_status(window: WebviewWindow, app: tauri::AppHandle) -> Result<CoreStatus, String> {
    authorize(&window)?;
    Ok(core_status_at(read_core_pointer(&app)))
}
#[tauri::command]
pub async fn desktop_core_choose_root(
    window: WebviewWindow,
    app: tauri::AppHandle,
    ...
    let selected = tauri::async_runtime::spawn_blocking(move || {
        picker_app
            .dialog()
            .file()
            .set_title("Choose a location for your Core")
            .blocking_pick_folder()
    })
    .await
    .map_err(|_| "Folder selection was interrupted.")?;
    authorize(&window)?;
    let Some(selected) = selected else {
        return Ok(core_status_at(read_core_pointer(&app)));
    };
    let parent = selected.into_path().map_err(|_| "Choose a local folder.")?;
    let (root, manifest) =
        tauri::async_runtime::spawn_blocking(move || create_core(&parent))
            .await
            .map_err(|_| "Core setup was interrupted.".to_string())??;
    write_core_pointer(&app, &root)?;
    Ok(CoreStatus {
        connected: true,
        path: Some(display(&root)),
        core_id: Some(manifest.core_id),
...
#[tauri::command]
pub async fn desktop_core_use_default(
    window: WebviewWindow,
    app: tauri::AppHandle,
) -> Result<CoreStatus, String> {
    authorize(&window)?;
    let documents = app
        .path()
        .document_dir()
        .map_err(|_| "The Documents location is unavailable.".to_string())?;
    let (root, manifest) = tauri::async_runtime::spawn_blocking(move || create_core(&documents))
        .await
        .map_err(|_| "Core setup was interrupted.".to_string())??;
    write_core_pointer(&app, &root)?;
    Ok(CoreStatus {
...
mod tests {
    ...
    #[test]
    fn creating_core_writes_a_dedicated_folder_manifest_and_compartments() {
        let t = Temp::new();
        let (root, manifest) = create_core(&t.0).unwrap();
        assert_eq!(root, t.0.join(CORE_FOLDER_NAME));
        assert_eq!(manifest.schema_version, CORE_MANIFEST_VERSION);
        assert!(!manifest.core_id.is_empty());
        for compartment in CORE_COMPARTMENTS {
            assert!(root.join(compartment).is_dir());
        }
        let status = core_status_at(Some(root));
        assert!(status.connected);
        assert_eq!(status.core_id.as_deref(), Some(manifest.core_id.as_str()));
    }
    #[test]
    fn creating_core_twice_is_idempotent_and_keeps_the_same_identity() {
        let t = Temp::new();
        let (_, first) = create_core(&t.0).unwrap();
        let (_, second) = create_core(&t.0).unwrap();
        assert_eq!(first.core_id, second.core_id);
    }
    #[test]
    fn creating_core_refuses_to_adopt_a_nonempty_unmanaged_folder() {
        let t = Temp::new();
        let existing = t.0.join(CORE_FOLDER_NAME);
        fs::create_dir(&existing).unwrap();
        fs::write(existing.join("unrelated.txt"), b"not core").unwrap();
        assert!(create_core(&t.0).is_err());
    }
    #[test]
    fn core_status_without_a_pointer_is_disconnected_and_silent() {
        let status = core_status_at(None);
        assert!(!status.connected);
        assert!(status.path.is_none());
        assert!(status.problem.is_none());
    }
    #[test]
    fn core_status_reports_a_problem_for_a_missing_manifest() {
        let t = Temp::new();
        let (root, _) = create_core(&t.0).unwrap();
        fs::remove_file(manifest_path(&root)).unwrap();
        let status = core_status_at(Some(root));
        assert!(!status.connected);
    ...
    #[test]
    fn non_linux_storage_status_never_invents_live_state() {
        #[cfg(not(target_os = "linux"))]
        {
            let status = linux_storage_status();
            assert!(!status.platform_supported);
            assert!(status.this_device.is_none());
            assert!(status.removable.is_empty());
        }
    }
    #[test]
    fn device_usage_parser_reads_bytes_from_df_output() {
        let device = parse_device_usage("        Size        Used       Avail\n 512110190592 128027545600  384082645").unwrap();
        assert_eq!(device.total_bytes, 512110190592);
        assert_eq!(device.used_bytes, 128027545600);
        assert_eq!(device.available_bytes, 384082645);
        assert!(parse_device_usage("Size Used Avail\n").is_none());
    }
    #[test]
    fn removable_volume_parser_keeps_only_removable_media() {
        let rows = "NAME=\"sda\" SIZE=\"256060514304\" RM=\"0\" MOUNTPOINT=\"\"\nNAME=\"sda1\" SIZE=\"536870912\" RM=\"0\" MOUNTPOINT=\"/boot\"\nNAME=\"sdb1\" SIZE=\"16008609792\" RM=\"1\" MOUNTPOINT=\"/media/usb\"\nNAME=\"sdc1\" SIZE=\"8000000000\" RM=\"1\" MOUNTPOINT=\"\"";
        let volumes = parse_removable_volumes(rows);
        assert_eq!(volumes.len(), 2);
        assert_eq!(volumes[0].name, "sdb1");
        assert_eq!(volumes[0].size_bytes, 16008609792);
        assert_eq!(volumes[0].mount_point.as_deref(), Some("/media/usb"));
        assert_eq!(volumes[1].name, "sdc1");
        assert!(volumes[1].mount_point.is_none());
    }
    #[test]
