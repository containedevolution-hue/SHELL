...
This is not a completed Chat-to-Tauri transport. Windows now compiles and fake-tests a private Rust driver connected to its exact Node child by a secret-authenticated inherited pipe; there is no listening socket or generic page IPC. The canonical Chat launcher, native caller/frame/navigation binding, native layout producer, liveness/geometry callbacks, and Linux/Windows runtime proof remain pending. See [the bridge boundary and integration requirements](IN-PROCESS-BRIDGE.md). Production remains unavailable and the registry remains empty. The [separate test-session procedure](../../os/CHAT-HYPRLAND-TEST.md) supplies Linux evidence collection; Windows still needs an equivalent non-mutating collector and physical acceptance. Public `v2.mjs` is unchanged, so consumer pins need no update.
