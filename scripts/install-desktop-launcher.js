...
function install({platform=process.platform, root=path.resolve(__dirname,'..'), home=os.homedir()} = {}) {
  ...
  fs.writeFileSync(desktop,`[Desktop Entry]\nType=Application\nName=SHELL Desktop (Development)\nComment=Shell desktop with native Core file navigation\nExec=${execQuote(launch)}\nIcon=system-file-manager\nTerminal=false\nCategories=System;FileManager;\nStartupNotify=true\n`);
  return desktop;
}
if(require.main===module){
  try{const result=install();spawnSync('update-desktop-database',[path.dirname(result)],{stdio:'ignore'});console.log(`Installed ${result}\nOpen SHELL Desktop (Development) from KDE’s application launcher. Keep the Shell checkout in place; the launcher uses its native build.`);}
