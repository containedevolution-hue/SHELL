...
- Chat is the neutral, independently installable conversation home owned by Apps. SHELL will launch the canonical Chat release, supply the adaptive Status Bar and bounded host capabilities, and provide Chat's one-slot [native desk](docs/surfaces/Chat-Native-Desk.md). Slice 3 has a default-off cross-platform registry, controller, exercised Hyprland adapter, authenticated in-process bridge, native geometry validator, deterministic Windows backend, and a private Rust Windows driver. The Rust-to-Node port is explicitly disconnected; Windows has no provider registration or runtime acceptance and therefore reports unavailable. The canonical Chat launcher, authenticated transport, live geometry producer, and hardware acceptance remain pending. The accepted desk will summon and switch complete downloaded provider applications without replacing them, preserve remote execution, and never become a Shell-specific Chat fork.
